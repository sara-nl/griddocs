...
// Very similar to WMS, but when a job is sent to cluster, it also moves a "status job" to the Log node
// The status node is checked when the job is finished
//
// Data flow:
...
//   - process_pilot_job
//     start the animation:
//     - create the status job and move it to the Log queue
//     - move the job to the cluster (CE)
//     - move the job to the worker node
//     - wait a bit and then remove the job
//     - Change the status icon to an icon with the check.
...
function update_queue() {
    ...
    status_jobs.transition()
        .duration(250)
        .attr({x : function(d,i) { return QueueX - (1+i/5.0)*envelope_size; },
               y : -envelope_size
              });
...
function process_pilot_job(job) {
    ...
    if (job_id >= 0) {
        ...
        {   // New with respect to wms.js, create the status job envelope and move it to the Log queue
        ...
        pilot_job.transition()
            ...
            .remove()
            .each("end", function() {
                // New w.r.t. wms.js: Change the icon of the "status envelope"
	        job.status_job.attr({"xlink:href":
                       (job.user_id==0?"img/envelope-yellow-checked.png":"img/envelope-blue-checked.png")})
...
function add_pilot_job_server(i) {
    ...
    pilot_job_system.transition().duration(2500)
        .attr("transform", "translate("+pos[0]+","+pos[1]+"),scale("+scale+")");
    add_icon(pilot_job_system, "img/computer.png", 0, icon_size/2, "WMS");
    add_icon(pilot_job_system, "img/computer.png", 0,-icon_size/2, "Log");
