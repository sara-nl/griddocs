...
var wfms_position = [user_position[0], user_position[1]+250];
var circle_positions = [[2*icon_size,0*icon_size],[2*icon_size,1*icon_size],[2*icon_size,2*icon_size],[2*icon_size,3*icon_size],[2*icon_size,4*icon_size]] ;
...
for (i=0; i<5; ++i) ComputerPositions.push([ComputerX, (i+0.5)*icon_size]);
...
function add_multiworkflow() {
    group = workflow.append("g");
    group
        .attr("transform", "translate("+200+","+height*0.5+"),scale("+0.5+")");
    group.append("rect")
        .attr("x", -icon_size)
        .attr("y", -icon_size)
        .attr("rx", icon_size/5.0)
        .attr("ry", icon_size/5.0)
        .attr("width", 14*icon_size)
        .attr("height", 10 * icon_size)
        .attr("style", "fill:none;stroke:black;stroke-width:5")
        .attr("transform", "scale(0.5)");
    group.append("line")
        .attr({x1:0,y1:2*icon_size,x2:6*icon_size, y2:2*icon_size})
        .attr("stroke","green")
        .attr("stroke-width", 3);
    //Draw lots o lines
    var start_pos = [1*icon_size, 2*icon_size];
    var end_pos = [5*icon_size, 2*icon_size];
    for (var i = 0; i<5; i++)  {
        group.append("line")
            .attr({x1:start_pos[0],y1:start_pos[1],x2:circle_positions[i][0], y2:circle_positions[i][1]})
            .attr("stroke","green")
            .attr("stroke-width", 3);
        group.append("line")
            .attr({x1:circle_positions[i][0]+2*icon_size,y1:circle_positions[i][1],x2:end_pos[0], y2:end_pos[1]})
            .attr("stroke","green")
            .attr("stroke-width", 3);
        group.append("line")
            .attr({x1:circle_positions[i][0],y1:circle_positions[i][1],x2:circle_positions[i][0]+2*icon_size, y2:circle_positions[i][1]})
            .attr("stroke","green")
            .attr("stroke-width", 3);
    }
            
        
    add_circle   (group, [0*icon_size,2*icon_size]);
    add_rectangle(group, [1*icon_size,2*icon_size]);

    for (var i = 0; i<5; i++)  add_circle   (group, circle_positions[i]);
    for (var i = 0; i<5; i++)  add_rectangle (group, [circle_positions[i][0]+1*icon_size,circle_positions[i][1]] );
    for (var i = 0; i<5; i++)  add_circle   (group, [circle_positions[i][0]+2*icon_size,circle_positions[i][1]] );


    add_rectangle(group, [5*icon_size,2*icon_size]);
    add_triangle (group, [6*icon_size,2*icon_size]);
...
function create_job(workflow_inst, i) {
    ...
    job_pos[1] = WorkflowInstances[workflow_inst.id].pos[1]+icon_size*scale/2*3;
...
function first_job_active(workflow_inst) {
    ...
    job_pos[1] = WorkflowInstances[workflow_inst.id].pos[1]+WorkflowInstances[0].scale*icon_size*2;
...
function get_first_data(workflow_inst, job, wn_id) {
    ...
    data_pos[1] = WorkflowInstances[workflow_inst.id].pos[1]+WorkflowInstances[0].scale*icon_size*2;
    
    scale = WorkflowInstances[workflow_inst.id].scale;
    for (var i = 0; i< 5 ; i++) {
        workflow_inst.data[0][1+i] = add_icon(svg, (workflow_inst.id?"img/disk-blue.png":"img/disk-green.png"), data_pos[0], data_pos[1]);
        workflow_inst.data[0][1+i]
...
function return_first_data(workflow_inst, wn_id) {
    ...
    for (var i = 0; i< 5 ; i++) {
        ...
        data_pos[1] = WorkflowInstances[workflow_inst.id].pos[1]+i*WorkflowInstances[workflow_inst.id].scale*icon_size;
...
function second_job_active(workflow_inst) {
    ...
    for (var i = 0; i<5; i++) {
        ...
        wn_pos = [0,0];
        wn_pos[0] = Clusters[0]["pos"][0]+Clusters[0]["scale"]*ComputerPositions[wn_id[i]][0];
        wn_pos[1] = Clusters[0]["pos"][1]+Clusters[0]["scale"]*ComputerPositions[wn_id[i]][1];
        ...
        job_pos[1] = WorkflowInstances[workflow_inst.id].pos[1]+i*WorkflowInstances[workflow_inst.id].scale*icon_size;
        
        scale = WorkflowInstances[workflow_inst.id].scale;
        add_icon(svg, "img/envelope-yellow.png", job_pos[0] + (1-scale)/2*icon_size, job_pos[1] + (1-scale)/2*icon_size)
            .attr({height:icon_size*scale, width: icon_size*scale});
        job[i] = add_icon(svg, "img/envelope-yellow.png", job_pos[0] + (1-scale)/2*icon_size, job_pos[1] + (1-scale)/2*icon_size);
        job[i]
            .attr({height:icon_size*scale, width: icon_size*scale})
            .transition()
            .delay(1000)
...
function get_second_data(workflow_inst, job, wn_id) {
    for(var i = 0 ; i< 5;i++){
        ...
        scale = WorkflowInstances[workflow_inst.id].scale;
        workflow_inst.data[1][0+i] = add_icon(svg, (workflow_inst.id?"img/disk-blue.png":"img/disk-green.png"), data_pos[0] + (1-scale)/2*icon_size, data_pos[1] + (1-scale)/2*icon_size)
        workflow_inst.data[1][0+i]
            .attr({height:icon_size*scale, width: icon_size*scale})
            .transition()
            .delay(2000)
            .duration(1000)
...
function return_second_data(workflow_inst, wn_id) {
    ...
    workflow_inst.data[1][0]
        .transition()
        .delay(3000)
        .each("end", function() { start_third_job(workflow_inst); });
}

function start_third_job(workflow_inst) {
    workflow_inst
        .transition()
        .delay(1000)
	.each("end", function() { third_job_active(workflow_inst); });
}

function third_job_active(workflow_inst) {
    wn_id = Math.floor((Math.random()*ComputerPositions.length));
    ...
    job_pos = [0,0];
    job_pos[0] = WorkflowInstances[workflow_inst.id].pos[0]+WorkflowInstances[0].scale*icon_size*5;
    job_pos[1] = WorkflowInstances[workflow_inst.id].pos[1]+WorkflowInstances[0].scale*icon_size*2;
    ...
    job
        ...
        .attr("x", wn_pos[0]-icon_size/2)
        .attr("y", wn_pos[1]-icon_size/2)
        .each("end", function() { get_third_data(workflow_inst, job, wn_id); });
}

function get_third_data(workflow_inst, job, wn_id) {
    ...
    for (var i =0 ; i<5; i++){
        ...
        scale = WorkflowInstances[workflow_inst.id].scale;
        workflow_inst.data[2][0+i] = add_icon(svg, (workflow_inst.id?"img/disk-blue.png":"img/disk-green.png"), data_pos[0] + (1-scale)/2*icon_size, data_pos[1] + (1-scale)/2*icon_size)
        workflow_inst.data[2][0+i]
            .attr({height:icon_size*scale, width: icon_size*scale})
            .transition()
            .delay(2000)
            .duration(1000)
    ...
    job
        ...
        .delay(4000)
        .remove();
    workflow_inst.data[2][0]
            .transition()
            .delay(3000)
            .each("end", function() { return_third_data(workflow_inst, wn_id); });
    
}

function return_third_data(workflow_inst, wn_id) {
    wn_pos = [0,0];
    wn_pos[0] = Clusters[0]["pos"][0] + Clusters[0]["scale"]*ComputerPositions[wn_id][0];
    wn_pos[1] = Clusters[0]["pos"][1] + Clusters[0]["scale"]*ComputerPositions[wn_id][1];
    ...
    data_pos = [0,0];
    data_pos[0] = WorkflowInstances[workflow_inst.id].pos[0]+6*WorkflowInstances[workflow_inst.id].scale*icon_size;
    data_pos[1] = WorkflowInstances[workflow_inst.id].pos[1]+2*WorkflowInstances[workflow_inst.id].scale*icon_size;
...
function return_to_sender(workflow_inst, disk) {
    workflow_inst.data[2][0]
        ...
        .delay(3000)
        .each("end", function() {
            add_icon(workflow_inst, "img/check.png", 7.25*icon_size, 0);
        })
        .remove();
...
function main() {
    ...
    add_cluster(0);
    workflow_definition = add_multiworkflow();
    workflow_inst2 = add_multiworkflow();
    workflow_inst2.id = 0;
    //workflow_inst3 = add_multiworkflow();
    //workflow_inst3.id = 1;
