...
//
// 1a. Send the data to the central storage element (send_data_to_cse)
// 1b. Move the data necessary for the job to the storage element of the cluster (send_data)
...
var CentralStoragePosition = [[QueueX-4*icon_size,height+3*icon_size]];
var NPackagesToCentralStorage = 5;
...
// New compared to data_transfer.js: First move the data to a central place
function send_data_to_cse() {
    for (var i=0; i<NPackagesToCentralStorage; ++i) {
        var data_i = add_icon(svg.select("#data"), "img/disk-blue.png", UserPositions[0][0], UserPositions[0][1]);
        data_i
	    .transition()
            .delay(200*i)
            .duration(1000)
            .attr("x", CentralStoragePosition[0][0]-10*i)
            .attr("y", CentralStoragePosition[0][1]);
    }
    data = add_icon(svg.select("#data"), "img/disk-blue.png", UserPositions[0][0], UserPositions[0][1]);
    // New compared to data_transfer.js: Move the data to the CSE and then to the SE
    data
	.transition()
        .delay(200*(NPackagesToCentralStorage-1))
        .duration(1000)
        .attr("x", CentralStoragePosition[0][0]-10*(NPackagesToCentralStorage-1))
        .attr("y", CentralStoragePosition[0][1])
        .transition()
        .delay(2000)
        .each("end", send_data);
...
function job_finished() {
	...
	.transition()
	.duration(1000)
	.attr("xlink:href", "img/disk-green.png")
    
    // Move the data to the storage position
	.transition()
	.delay(1000)
	.duration(1000)
	.attr("x", StoragePosition[0][0])
    ...
    // Then move the data to the central storage position
	.transition()
	.delay(2000)
	.duration(1000)
	.attr("x", CentralStoragePosition[0][0] - 10 * NPackagesToCentralStorage)
	.attr("y", CentralStoragePosition[0][1]);
...
function user_get_data() {
    var result = svg.append("line")
		...
		.transition()
		.duration(1000)
		.attr("x2", data[0][0].x.animVal.value+icon_size/2)
		.attr("y2", data[0][0].y.animVal.value+icon_size/2)
...
function main() {
    ...
    svg.append("g").attr("id", "central_storage");
    ...
    add_icons(d3.select("#central_storage"), "img/database.png", CentralStoragePosition, "CSE");
