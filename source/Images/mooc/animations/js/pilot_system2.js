...
//   - create_pool_job (the jobs is pushed to PoolServer[0]["queued_jobs"])
...
function create_pool_job(user_id) {
    {
        var pjs_jobs = PoolServer[0]["svg"].select("#submitted_jobs");
...
function process_jobs(cluster_id, computer_id) {
    ...
    setTimeout(process_jobs, 2000 + 5000*Math.random(), cluster_id, computer_id);
}
...
function add_pool_job_server(i) {
    ...
    pool_job_system.append("g").attr("id", "submitted_jobs");
