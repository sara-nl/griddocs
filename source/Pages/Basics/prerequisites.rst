...
The Grid is a cooperation of many different clusters and research organisations. The system identifies you and your work through the usage of **Grid certificates** and **Virtual Organisations** (VOs), instead of a centralised user management providing a username and a password. 
...
The User Interface account will provide you with the proper environment to submit your jobs to the Grid. The Grid certificate is required to authorize you for using the Grid. Finally, the :abbr:`VO (Virtual Organisation)` membership is based on your research domain (e.g. ``lsgrid`` for Life Scientists) and determines which resources you can use.
...
* If you work in Life Sciences sector and your local institutional cluster is part of the :ref:`lsg` (see :ref:`lsg-clusters`), then you can get an account to access the local user interface. Please send your request to your designated site admin (see :ref:`local contacts <lsg-dsa>`) or contact us at helpdesk@surfsara.nl.

* For any other case, we will give you an account on the SURFsara Grid :abbr:`UI (User Interface)` (server name: ``ui.grid.sara.nl``). For this, you need to contact us at helpdesk@surfsara.nl.

.. sidebar:: More about Work Environment-Remote access?

		.. seealso:: Please note that the :abbr:`UI (User Interface)` is simply a Linux machine and working on it requires some familiarity with linux commands and remote access methods. If you need help with this, check out our mooc video :ref:`remote-access`.
